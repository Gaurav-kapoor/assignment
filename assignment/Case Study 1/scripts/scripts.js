$(function () {
    ...
    $('.order-summary .slider-wrapper').slick({
        dots: false,
        arrows: true,
        infinite: false,
        slidesToShow: 3,
    ...
    $('.order-summary .slider-wrapper .text-wrap').on('click', function(){
        let order = $(this).index();
        $('.order-summary .order-wrapper .order-details').eq(order).fadeIn(1000).siblings().fadeOut(800);
    });
    ...
    $('.products .payment form .confirm').on('click', function () {
        ...
        $(this).parents('.products').find('.breadcrumb ul li').eq(3).addClass('text-success').siblings().removeClass('text-success');
    });

    $('.products .confirm-order .checkout').on('click', function () {
        $('.products .confirm-order .checkout').hide();
